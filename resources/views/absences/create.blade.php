...
@section('content')
                    ...
                    <form method="POST" action="{{ route('absences.store') }}" enctype="multipart/form-data"
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="employee_id" class="block text-sm font-medium text-gray-700">Funcionário
                                        *</label>
                                    <select name="employee_id" id="employee_id" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('employee_id') border-red-300 @enderror">
                                ...
                                <div>
                                    <label for="absence_type" class="block text-sm font-medium text-gray-700">Tipo de
                                        Ausência *</label>
                                    <select name="absence_type" id="absence_type" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('absence_type') border-red-300 @enderror">
                                        <option value="">Selecione o tipo...</option>
                                        <option value="sick_leave"
                                            {{ old('absence_type') == 'sick_leave' ? 'selected' : '' }}>Atestado Médico
                                        </option>
                                        <option value="vacation" {{ old('absence_type') == 'vacation' ? 'selected' : '' }}>
                                            Férias</option>
                                        <option value="maternity_leave"
                                            {{ old('absence_type') == 'maternity_leave' ? 'selected' : '' }}>Licença
                                            Maternidade</option>
                                        <option value="paternity_leave"
                                            {{ old('absence_type') == 'paternity_leave' ? 'selected' : '' }}>Licença
                                            Paternidade</option>
                                        <option value="bereavement"
                                            {{ old('absence_type') == 'bereavement' ? 'selected' : '' }}>Luto (Art. 473, I
                                            CLT)</option>
                                        <option value="marriage" {{ old('absence_type') == 'marriage' ? 'selected' : '' }}>
                                            Casamento (Art. 473, II CLT)</option>
                                        <option value="blood_donation"
                                            {{ old('absence_type') == 'blood_donation' ? 'selected' : '' }}>Doação de
                                            Sangue (Art. 473, IV CLT)</option>
                                        <option value="military_service"
                                            {{ old('absence_type') == 'military_service' ? 'selected' : '' }}>Serviço
                                            Militar</option>
                                        <option value="jury_duty"
                                            {{ old('absence_type') == 'jury_duty' ? 'selected' : '' }}>Júri (Art. 473, VII
                                            CLT)</option>
                                        <option value="witness_testimony"
                                            {{ old('absence_type') == 'witness_testimony' ? 'selected' : '' }}>Testemunha
                                            (Art. 473, VI CLT)</option>
                                        <option value="union_activity"
                                            {{ old('absence_type') == 'union_activity' ? 'selected' : '' }}>Atividade
                                            Sindical</option>
                                        <option value="study_leave"
                                            {{ old('absence_type') == 'study_leave' ? 'selected' : '' }}>Licença para
                                            Estudos</option>
                                        <option value="unpaid_leave"
                                            {{ old('absence_type') == 'unpaid_leave' ? 'selected' : '' }}>Licença sem
                                            Vencimento</option>
                                        <option value="unjustified"
                                            {{ old('absence_type') == 'unjustified' ? 'selected' : '' }}>Falta
                                            Injustificada</option>
                                        <option value="other" {{ old('absence_type') == 'other' ? 'selected' : '' }}>
                                            Outros Motivos</option>
                                    </select>
                                    @error('absence_type')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    <label for="total_days" class="block text-sm font-medium text-gray-700">Total de
                                        Dias</label>
                                    <input type="number" name="total_days" id="total_days"
                                        value="{{ old('total_days', 1) }}" min="1" readonly
                                ...
                                <div class="md:col-span-2">
                                    <label for="justification" class="block text-sm font-medium text-gray-700">Justificativa
                                        *</label>
                                    <textarea name="justification" id="justification" rows="3" required placeholder="Descreva o motivo da ausência..."
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="document_number" id="document_number"
                                        value="{{ old('document_number') }}" placeholder="Ex: Número do atestado médico"
                                ...
                                <div>
                                    <label for="document_path" class="block text-sm font-medium text-gray-700">Anexar
                                        Documento</label>
                                ...
                                <div>
                                    <label for="document_date" class="block text-sm font-medium text-gray-700">Data do
                                        Documento</label>
                        ...
                        <div class="border-b border-gray-200 pb-6" id="medical_info" style="display: none;">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="doctor_name" class="block text-sm font-medium text-gray-700">Nome do
                                        Médico</label>
                                ...
                                <div>
                                    <label for="doctor_crm" class="block text-sm font-medium text-gray-700">CRM do
                                        Médico</label>
                                    <input type="text" name="doctor_crm" id="doctor_crm"
                                        value="{{ old('doctor_crm') }}" placeholder="Ex: CRM/SP 123456"
                                ...
                                <div>
                                    <label for="medical_code" class="block text-sm font-medium text-gray-700">Código
                                        CID</label>
                                    <input type="text" name="medical_code" id="medical_code"
                                        value="{{ old('medical_code') }}" placeholder="Ex: M54.5"
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center">
                                    <input type="checkbox" name="paid_absence" id="paid_absence" value="1"
                                        {{ old('paid_absence', true) ? 'checked' : '' }}
                                ...
                                <div>
                                    <label for="discount_amount" class="block text-sm font-medium text-gray-700">Valor do
                                        Desconto (R$)</label>
    ...
    <script>
        ...
        document.getElementById('absence_type').addEventListener('change', function() {
            ...
            const paidTypes = ['sick_leave', 'vacation', 'maternity_leave', 'paternity_leave', 'bereavement',
                'marriage', 'blood_donation', 'jury_duty', 'witness_testimony'
            ];
