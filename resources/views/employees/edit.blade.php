...
@section('header')
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Funcionário: ') . $employee->user->name }}
        </h2>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="cpf" id="cpf"
                                        value="{{ old('cpf', $employee->cpf) }}" required placeholder="000.000.000-00"
                                        maxlength="14"
                                ...
                                <div>
                                    ...
                                    <select name="gender" id="gender"
                                        ...
                                        <option value="M"
                                            {{ old('gender', $employee->gender) == 'M' ? 'selected' : '' }}>Masculino
                                        </option>
                                        <option value="F"
                                            {{ old('gender', $employee->gender) == 'F' ? 'selected' : '' }}>Feminino
                                        </option>
                                        <option value="O"
                                            {{ old('gender', $employee->gender) == 'O' ? 'selected' : '' }}>Outro</option>
                                    </select>
                                    @error('gender')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="text" name="phone" id="phone"
                                        value="{{ old('phone', $employee->phone) }}" placeholder="(00) 00000-0000"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="registration_number" id="registration_number"
                                        value="{{ old('registration_number', $employee->registration_number) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="text" name="pis_pasep" id="pis_pasep"
                                        value="{{ old('pis_pasep', $employee->pis_pasep) }}" placeholder="000.00000.00-0"
                                        maxlength="14"
                                ...
                                <div>
                                    <label for="dismissal_date" class="block text-sm font-medium text-gray-700">Data de
                                        Demissão</label>
                                    <input type="date" name="dismissal_date" id="dismissal_date"
                                        value="{{ old('dismissal_date', $employee->dismissal_date?->format('Y-m-d')) }}"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" name="weekly_hours" id="weekly_hours"
                                        value="{{ old('weekly_hours', $employee->workSchedule->weekly_hours) }}" required
                                        min="1" max="44"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="meal_break_minutes" id="meal_break_minutes"
                                        value="{{ old('meal_break_minutes', $employee->workSchedule->meal_break_minutes) }}"
                                        required min="30" max="120"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="rfid_card" id="rfid_card"
                                        value="{{ old('rfid_card', $employee->rfid_card) }}"
                                ...
                                <div class="flex items-center space-x-6">
                                    <div class="flex items-center">
                                        <input type="checkbox" name="exempt_time_control" id="exempt_time_control"
                                            value="1"
                                            {{ old('exempt_time_control', $employee->exempt_time_control) ? 'checked' : '' }}
                                    ...
                                    <div class="flex items-center">
                                        <input type="checkbox" name="has_meal_break" id="has_meal_break" value="1"
                                            {{ old('has_meal_break', $employee->has_meal_break) ? 'checked' : '' }}
                                            class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="state" id="state"
                                        ...
                                        <option value="">Selecione...</option>
                                        <option value="AC"
                                            {{ old('state', $employee->state) == 'AC' ? 'selected' : '' }}>Acre</option>
                                        <option value="AL"
                                            {{ old('state', $employee->state) == 'AL' ? 'selected' : '' }}>Alagoas</option>
                                        <option value="AP"
                                            {{ old('state', $employee->state) == 'AP' ? 'selected' : '' }}>Amapá</option>
                                        <option value="AM"
                                            {{ old('state', $employee->state) == 'AM' ? 'selected' : '' }}>Amazonas
                                        </option>
                                        <option value="BA"
                                            {{ old('state', $employee->state) == 'BA' ? 'selected' : '' }}>Bahia</option>
                                        <option value="CE"
                                            {{ old('state', $employee->state) == 'CE' ? 'selected' : '' }}>Ceará</option>
                                        <option value="DF"
                                            {{ old('state', $employee->state) == 'DF' ? 'selected' : '' }}>Distrito Federal
                                        </option>
                                        <option value="ES"
                                            {{ old('state', $employee->state) == 'ES' ? 'selected' : '' }}>Espírito Santo
                                        </option>
                                        <option value="GO"
                                            {{ old('state', $employee->state) == 'GO' ? 'selected' : '' }}>Goiás</option>
                                        <option value="MA"
                                            {{ old('state', $employee->state) == 'MA' ? 'selected' : '' }}>Maranhão
                                        </option>
                                        <option value="MT"
                                            {{ old('state', $employee->state) == 'MT' ? 'selected' : '' }}>Mato Grosso
                                        </option>
                                        <option value="MS"
                                            {{ old('state', $employee->state) == 'MS' ? 'selected' : '' }}>Mato Grosso do
                                            Sul</option>
                                        <option value="MG"
                                            {{ old('state', $employee->state) == 'MG' ? 'selected' : '' }}>Minas Gerais
                                        </option>
                                        <option value="PA"
                                            {{ old('state', $employee->state) == 'PA' ? 'selected' : '' }}>Pará</option>
                                        <option value="PB"
                                            {{ old('state', $employee->state) == 'PB' ? 'selected' : '' }}>Paraíba</option>
                                        <option value="PR"
                                            {{ old('state', $employee->state) == 'PR' ? 'selected' : '' }}>Paraná</option>
                                        <option value="PE"
                                            {{ old('state', $employee->state) == 'PE' ? 'selected' : '' }}>Pernambuco
                                        </option>
                                        <option value="PI"
                                            {{ old('state', $employee->state) == 'PI' ? 'selected' : '' }}>Piauí</option>
                                        <option value="RJ"
                                            {{ old('state', $employee->state) == 'RJ' ? 'selected' : '' }}>Rio de Janeiro
                                        </option>
                                        <option value="RN"
                                            {{ old('state', $employee->state) == 'RN' ? 'selected' : '' }}>Rio Grande do
                                            Norte</option>
                                        <option value="RS"
                                            {{ old('state', $employee->state) == 'RS' ? 'selected' : '' }}>Rio Grande do
                                            Sul</option>
                                        <option value="RO"
                                            {{ old('state', $employee->state) == 'RO' ? 'selected' : '' }}>Rondônia
                                        </option>
                                        <option value="RR"
                                            {{ old('state', $employee->state) == 'RR' ? 'selected' : '' }}>Roraima</option>
                                        <option value="SC"
                                            {{ old('state', $employee->state) == 'SC' ? 'selected' : '' }}>Santa Catarina
                                        </option>
                                        <option value="SP"
                                            {{ old('state', $employee->state) == 'SP' ? 'selected' : '' }}>São Paulo
                                        </option>
                                        <option value="SE"
                                            {{ old('state', $employee->state) == 'SE' ? 'selected' : '' }}>Sergipe</option>
                                        <option value="TO"
                                            {{ old('state', $employee->state) == 'TO' ? 'selected' : '' }}>Tocantins
                                        </option>
                                    </select>
                                    @error('state')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="zip_code" id="zip_code"
                                        value="{{ old('zip_code', $employee->zip_code) }}" placeholder="00000-000"
                                        maxlength="9"
