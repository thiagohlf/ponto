                ...
                <div class="p-6">
                    <form method="GET" action="{{ route('overtime.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <select name="overtime_type" id="overtime_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="">Todos os tipos</option>
                                <option value="daily_overtime" {{ request('overtime_type') == 'daily_overtime' ? 'selected' : '' }}>Hora Extra Diária</option>
                                <option value="weekend_work" {{ request('overtime_type') == 'weekend_work' ? 'selected' : '' }}>Trabalho em Fim de Semana</option>
                                <option value="holiday_work" {{ request('overtime_type') == 'holiday_work' ? 'selected' : '' }}>Trabalho em Feriado</option>
                                <option value="night_shift" {{ request('overtime_type') == 'night_shift' ? 'selected' : '' }}>Adicional Noturno</option>
                                <option value="compensatory" {{ request('overtime_type') == 'compensatory' ? 'selected' : '' }}>Banco de Horas</option>
                                <option value="emergency" {{ request('overtime_type') == 'emergency' ? 'selected' : '' }}>Emergência</option>
                            </select>
                        </div>
                        ...
                        <div class="md:col-span-5 flex justify-end space-x-2">
                            ...
                            <a href="{{ route('overtime.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                                Limpar
                            </a>
                        </div>
                    </form>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div class="text-center">
                                <div class="text-2xl font-bold text-green-600">{{ number_format($overtime->sum('total_minutes') / 60, 1) }}h</div>
                                <div class="text-sm text-gray-500">Total de Horas</div>
                            </div>
                            <div class="text-center">
                                <div class="text-2xl font-bold text-yellow-600">{{ $overtime->where('status', 'pending')->count() }}</div>
                                <div class="text-sm text-gray-500">Pendentes</div>
                            </div>
                            <div class="text-center">
                                <div class="text-2xl font-bold text-purple-600">R$ {{ number_format($overtime->sum('calculated_amount'), 2, ',', '.') }}</div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($overtime as $record)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ $record->work_date->format('d/m/Y') }}</div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                    ...
                                                    @switch($record->overtime_type)
                                                        @case('daily_overtime') Hora Extra Diária @break
                                                        @case('weekend_work') Fim de Semana @break
                                                        @case('holiday_work') Feriado @break
                                                        @case('night_shift') Adicional Noturno @break
                                                        @case('compensatory') Banco de Horas @break
                                                        @case('emergency') Emergência @break
                                                        @default {{ $record->overtime_type }}
                                                    @endswitch
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">{{ number_format($record->total_minutes / 60, 1) }}h</div>
                                                @if($record->night_shift_applicable && $record->night_shift_minutes > 0)
                                                    <div class="text-xs text-gray-500">{{ number_format($record->night_shift_minutes / 60, 1) }}h noturno</div>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">R$ {{ number_format($record->calculated_amount, 2, ',', '.') }}</div>
                                                <div class="text-xs text-gray-500">{{ number_format($record->overtime_multiplier, 2) }}x</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('overtime.show', $record) }}" class="text-blue-600 hover:text-blue-900">
                                                        Ver
                                                    </a>
                                                    
                                                    @can('gerenciar_horas_extras')
                                                        @if($record->status === 'pending')
                                                            <a href="{{ route('overtime.edit', $record) }}" class="text-indigo-600 hover:text-indigo-900">
                                                                Editar
                                                            </a>
                                                        @endif
                                                    @endcan
