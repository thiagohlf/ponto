                ...
                <div class="p-6">
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-yellow-700">
                                    @if (auth()->user()->isSupervisor() || auth()->user()->isHR() || auth()->user()->isAdmin())
                                        <p>Este registro será marcado como "manual" e ficará pendente de aprovação conforme
                                            a legislação trabalhista brasileira.</p>
                                    @else
                                        <p>Esta solicitação de ajuste de ponto será enviada para aprovação do seu supervisor
                                            conforme a legislação trabalhista brasileira.</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <form method="POST" action="{{ route('time-records.store') }}" class="space-y-6"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <select name="employee_id" id="employee_id" required
                                        ...
                                        @foreach ($employees as $employee)
                                            <option value="{{ $employee->id }}"
                                                ...
                                                {{ $employee->user->name }} - {{ $employee->registration_number }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('employee_id')
                                ...
                                <div>
                                    <label for="identification_method"
                                        class="block text-sm font-medium text-gray-700">Método de Identificação</label>
                                    <select name="identification_method" id="identification_method"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('identification_method') border-red-300 @enderror">
                                        <option value="manual"
                                            {{ old('identification_method', 'manual') == 'manual' ? 'selected' : '' }}>
                                            Manual (Supervisor)</option>
                                        <option value="web_login"
                                            {{ old('identification_method') == 'web_login' ? 'selected' : '' }}>Web Login
                                        </option>
                                        <option value="web_biometric"
                                            {{ old('identification_method') == 'web_biometric' ? 'selected' : '' }}>Web
                                            Biométrico</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="time" name="record_time" id="record_time"
                                        value="{{ old('record_time', date('H:i')) }}" required
                                ...
                                <div>
                                    ...
                                    <select name="record_type" id="record_type" required
                                        ...
                                        <option value="entry" {{ old('record_type') == 'entry' ? 'selected' : '' }}>
                                            Entrada</option>
                                        <option value="exit" {{ old('record_type') == 'exit' ? 'selected' : '' }}>Saída
                                        </option>
                                        <option value="meal_start"
                                            {{ old('record_type') == 'meal_start' ? 'selected' : '' }}>Saída para Almoço
                                        </option>
                                        <option value="meal_end" {{ old('record_type') == 'meal_end' ? 'selected' : '' }}>
                                            Retorno do Almoço</option>
                                        <option value="break_start"
                                            {{ old('record_type') == 'break_start' ? 'selected' : '' }}>Início de Pausa
                                        </option>
                                        <option value="break_end"
                                            {{ old('record_type') == 'break_end' ? 'selected' : '' }}>Fim de Pausa</option>
                                    </select>
                                    @error('record_type')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" name="latitude" id="latitude" value="{{ old('latitude') }}"
                                        step="0.00000001" min="-90" max="90" placeholder="-23.5505199"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="longitude" id="longitude" value="{{ old('longitude') }}"
                                        step="0.00000001" min="-180" max="180" placeholder="-46.6333094"
                                ...
                                <div class="md:col-span-2">
                                    <button type="button" id="getLocation"
                                        class="inline-flex items-center px-3 py-2 border border-green-700 shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-green-800 hover:bg-green-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        ...
                        <div>
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label for="change_justification"
                                        class="block text-sm font-medium text-gray-700">Justificativa para Registro Manual
                                        *</label>
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">Esta justificativa é obrigatória para registros
                                        manuais conforme a legislação trabalhista.</p>
                                            ...
                                            <div class="text-sm text-gray-600">
                                                <label for="attachments"
                                                    ...
                                                    <input id="attachments" name="attachments[]" type="file"
                                                        class="sr-only" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx">
                                                </label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function displaySelectedFiles(files) {
                ...
                Array.from(files).forEach((file, index) => {
                    ...
                    fileItem.className =
                        `flex items-center justify-between p-2 bg-gray-50 rounded border ${error ? 'border-red-300' : 'border-gray-200'}`;
