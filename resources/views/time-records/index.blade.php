...
@section('header')
    <div class="flex justify-between items-center">
        ...
        @can('gerenciar_registros_ponto')
            <a href="{{ route('time-records.create') }}"
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('time-records.index') }}"
                        class="grid grid-cols-1 {{ $employees->count() > 0 ? 'md:grid-cols-6' : 'md:grid-cols-5' }} gap-4">
                        ...
                        <div>
                            <label for="identification_method"
                                class="block text-sm font-medium text-gray-700">Método</label>
                            <select name="identification_method" id="identification_method"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="">Todos os métodos</option>
                                <option value="web_login"
                                    {{ request('identification_method') == 'web_login' ? 'selected' : '' }}>Web Login
                                </option>
                                <option value="web_biometric"
                                    {{ request('identification_method') == 'web_biometric' ? 'selected' : '' }}>Web
                                    Biométrico</option>
                                <option value="manual" {{ request('identification_method') == 'manual' ? 'selected' : '' }}>
                                    Manual</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="record_type" id="record_type"
                                ...
                                <option value="entry" {{ request('record_type') == 'entry' ? 'selected' : '' }}>Entrada
                                </option>
                                <option value="exit" {{ request('record_type') == 'exit' ? 'selected' : '' }}>Saída
                                </option>
                                <option value="meal_start" {{ request('record_type') == 'meal_start' ? 'selected' : '' }}>
                                    Saída Almoço</option>
                                <option value="meal_end" {{ request('record_type') == 'meal_end' ? 'selected' : '' }}>
                                    Retorno Almoço</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="valid" {{ request('status') == 'valid' ? 'selected' : '' }}>Válidos
                                </option>
                                <option value="pending_approval"
                                    {{ request('status') == 'pending_approval' ? 'selected' : '' }}>Pendentes</option>
                                <option value="invalid" {{ request('status') == 'invalid' ? 'selected' : '' }}>Inválidos
                                </option>
                            </select>
                        </div>

                        <div
                            class="{{ $employees->count() > 0 ? 'md:col-span-6' : 'md:col-span-5' }} flex justify-end space-x-2">
                            <button type="submit"
                                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                Filtrar
                            </button>
                                        ...
                                        <tr class="hover:bg-gray-50">
                                                ...
                                                @switch($record->identification_method)
                                                    @case('web_login')
                                                        <span class="flex items-center">
                                                            <svg class="h-4 w-4 mr-1 text-blue-500" fill="currentColor"
                                                                viewBox="0 0 20 20">
                                                    ...
                                                    @case('web_biometric')
                                                        <span class="flex items-center">
                                                            <svg class="h-4 w-4 mr-1 text-green-500" fill="currentColor"
                                                                viewBox="0 0 20 20">
                                                                <path fill-rule="evenodd"
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                @if ($record->ip_address)
                                                    <span class="text-xs text-gray-500">{{ $record->ip_address }}</span>
                                                @else
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('time-records.show', $record) }}"
                                                    ...
                                                    @can('aprovar_registros_ponto')
                                                        @if ($record->status === 'pending_approval')
                                                            <form method="POST"
                                                                action="{{ route('time-records.approve', $record) }}"
                                                                class="inline">
                                                                @csrf
                                                                @method('PATCH')
                                                                <button type="submit"
                                                    ...
                                                    @can('gerenciar_registros_ponto')
                                                        <a href="{{ route('time-records.edit', $record) }}"
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                @if (request()->hasAny(['employee_id', 'identification_method', 'date_from', 'date_to', 'record_type', 'status']))
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <form id="rejectForm" method="POST">
                    ...
                    @method('PATCH')
                    <div class="mb-4">
                        <label for="rejection_reason" class="block text-sm font-medium text-gray-700">Motivo da
                            Rejeição</label>
