                ...
                <div class="lg:col-span-2">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                @if ($company->contact_data && isset($company->contact_data['email']))
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Email</label>
                                        <p class="mt-1 text-sm text-gray-900">{{ $company->contact_data['email'] }}</p>
                                    </div>
                                @endif

                                @if ($company->contact_data && isset($company->contact_data['phone']))
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Telefone</label>
                                        <p class="mt-1 text-sm text-gray-900">{{ $company->contact_data['phone'] }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="p-6">
                            ...
                            @if ($company->address_data)
                                <div class="text-sm text-gray-900">
                                    <p>{{ $company->address_data['street'] ?? '' }},
                                        {{ $company->address_data['number'] ?? '' }}</p>
                                    @if (isset($company->address_data['complement']) && $company->address_data['complement'])
                                        <p>{{ $company->address_data['complement'] }}</p>
                                    @endif
                                    <p>{{ $company->address_data['neighborhood'] ?? '' }}</p>
                                    <p>{{ $company->address_data['city'] ?? '' }}/{{ $company->address_data['state'] ?? '' }}
                                        - {{ $company->address_data['zip_code'] ?? '' }}</p>
                                </div>
                            @else
                                <p class="text-sm text-gray-500">Endereço não cadastrado</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mt-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Configurações de
                                        Tolerância</label>
                                    <p class="mt-1 text-sm text-gray-900">Definidas nos horários de trabalho</p>
                                </div>
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600">Horários de Trabalho:</span>
                                    <span
                                        class="text-sm font-medium text-gray-900">{{ $company->workSchedules->count() }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <div class="space-y-2">
                                ...
                                @can('gerenciar_horarios')
                                    <a href="{{ route('work-schedules.index', ['company_id' => $company->id]) }}"
                                        class="w-full flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-gray-50 rounded-md hover:bg-gray-100">
                                        <svg class="mr-3 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
